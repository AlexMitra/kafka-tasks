...
@Component
@Slf4j
public class VehicleCoordinatesListener1 {

	@KafkaListener(topicPartitions = @TopicPartition(topic = "${app.vehicle-coordinates-topic.name}",
			partitions = {"0"}), groupId = "${spring.kafka.consumer.group-id}")
	public void onConsume(VehicleDTO vehicleDTO, @Header(KafkaHeaders.RECEIVED_PARTITION) int partition) {
		log.info("[TRACKER-1] Received vehicle with coordinates for tracking '{}' from partition {}", vehicleDTO,
				partition);
	}
}
